 ...
 * façon.
 */
export function ServiceSearch({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  ...
  return (
    <div className="mt-8">
      <label htmlFor={idListe} className="mb-2 block text-sm font-semibold text-white">
        {dict.search.label}
      </label>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full min-w-0 rounded-2xl border border-white/15 bg-white/8 py-4 pe-4 ps-12 text-base text-white shadow-lg backdrop-blur-sm transition placeholder:text-slate-400 focus:border-aqua-400 focus:bg-white/12 focus:outline-none focus:ring-4 focus:ring-aqua-500/25"
        />
      </div>
      ...
      {resultats.length > 0 ? (
        <ul className="mt-3 overflow-hidden rounded-2xl border border-white/12 bg-ink-900/85 shadow-2xl backdrop-blur-xl">
          {resultats.map((service) => (
            <li key={service.slug} className="border-b border-white/6 last:border-b-0">
              <Link
                href={localePath(locale, `services/${service.slug}`)}
                className="flex items-center gap-3 px-4 py-3.5 transition hover:bg-white/6"
              >
                <span
                ...
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-semibold text-white">{service.name[locale]}</span>
      ...
      {chercheSansTrouver ? (
        <div className="mt-3 rounded-2xl border border-white/12 bg-ink-900/85 p-4 backdrop-blur-xl">
          <p className="text-sm text-slate-300">{dict.search.none}</p>
          <Link
            href={localePath(locale, "demande")}
            className="mt-3 inline-flex items-center gap-2 text-sm font-bold text-aqua-300 transition hover:text-aqua-200"
          >
